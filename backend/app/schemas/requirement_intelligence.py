...
class BaseAIOutput(BaseModel):
    ...
    evidence: Optional[str] = Field("", description="Verbatim text quote from the RFP as source proof")
    reasoning: Optional[str] = Field("", description="LLM logic explaining why this item was extracted")
    validation_status: str = Field("Not Validated", description="Status of validation rules validation")
...
class RequirementExtractionOutput(BaseAIOutput):
    ...
    category: str = Field(..., description="e.g. Technical, Legal, Financial, Operational, Security, etc.")
    priority: str = Field(..., description="Priority level: High, Medium, Low")
    req_type: str = Field(..., description="Type: Functional or Non-Functional")
    mandatory: bool = Field(..., description="Whether the requirement is mandatory or optional")
    ...
    assigned_departments: List[str] = Field(..., description="List of responsible departments")
...
class DeliverableExtractionOutput(BaseAIOutput):
    ...
    mandatory: bool = Field(..., description="Whether this deliverable is mandatory")
    responsible_department: Optional[str] = Field(None, description="Responsible department")
    related_requirements: List[str] = Field(..., description="List of related requirement temp IDs")
...
class ComplianceItemOutput(BaseAIOutput):
    ...
    status: str = Field(..., description="Status: Unknown, Compliant, Non-Compliant")
    ...
    priority: str = Field(..., description="Priority level: High, Medium, Low")
    blocking: bool = Field(..., description="Whether missing this blocks proposal submission")
...
class RiskOutput(BaseAIOutput):
    ...
    related_requirements: List[str] = Field(..., description="Related requirement temp IDs")
...
class AssumptionOutput(BaseAIOutput):
    ...
    category: str = Field(..., description="Business, Technical, Operational, Contractual")
    is_explicit: bool = Field(..., description="True if explicitly stated, False if inferred")
...
class RequirementIntelligenceOutput(BaseModel):
    requirements: List[RequirementExtractionOutput] = Field(..., description="List of extracted requirements")
    deliverables: List[DeliverableExtractionOutput] = Field(..., description="List of extracted deliverables")
    evaluation_criteria: List[EvaluationCriteriaOutput] = Field(..., description="List of extracted evaluation criteria")
    submission_instructions: List[SubmissionInstructionOutput] = Field(..., description="List of extracted submission instructions")
    compliance_items: List[ComplianceItemOutput] = Field(..., description="List of extracted compliance obligations")
    risks: List[RiskOutput] = Field(..., description="List of extracted risks")
    assumptions: List[AssumptionOutput] = Field(..., description="List of extracted assumptions")
    clarification_questions: List[ClarificationQuestionOutput] = Field(..., description="List of extracted clarification questions")
    knowledge_graph_edges: List[KnowledgeGraphEdgeOutput] = Field(..., description="List of extracted relationship graph edges")
